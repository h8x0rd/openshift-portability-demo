...
simulated="$(oc get managedcluster "$cluster" \
  -o jsonpath='{.metadata.annotations.demo\.portability/simulated-failure}')"
[[ "$simulated" == true ]] || \
...
idx="$(oc get managedcluster "$cluster" -o json | jq -r '
  .spec.taints // []
  | to_entries[]
  | select(.value.key=="cluster.open-cluster-management.io/unreachable")
  | .key' | head -1)"
[[ -n "$idx" ]] || die "$cluster has no unreachable taint."

patch="[{\"op\":\"remove\",\"path\":\"/spec/taints/$idx\"}]"
oc patch managedcluster "$cluster" --type=json -p="$patch" >/dev/null
oc annotate managedcluster "$cluster" demo.portability/simulated-failure- >/dev/null
